 ...
 */
@Controller
@RequestMapping("/tbAttachmentJsz")
public class TbAttachmentJszController {

	 private final Logger logger = LoggerFactory.getLogger(this.getClass());
	// 支持上传证件详情图的最大数量
	private static final int IMAGEMAXCOUNT = 10;
	@Resource
    private TbAttachmentJszService tbAttachmentJszService;
	@Resource
    ...
    public JSONObject doAddOld(HttpServletRequest request,String cId,String imgFlag) {
    	...
    	// 3.添加附件信息
    	if (imgList.size() > 0) {
    		TbAttachmentJsz tbAttachmentJsz = new TbAttachmentJsz();
    		tbAttachmentJsz.setClId(cId);
    		tbAttachmentJsz.setUlUserId(user.getId());
    		tbAttachmentJsz.setUlUserName(user.getRealName());
    		try {
    			// 执行添加操作
    			Execution<TbAttachmentJsz> execution = tbAttachmentJszService
    					.add(tbAttachmentJsz, imgList);
    			...
    			json.put("success", false);
    			json.put("errMsg", e.toString());
    			logger.error("添加驾驶证上传记录时出现异常！error message:"+e.getMessage(),e);
    			return json;
    		}
    	}
    	else {
	...
	public JSONObject uploadSingle(HttpServletRequest request) {
		...
		try {
			...
			logger.error("保存驾驶证信息失败！error massage："+e.getMessage(),e);
			json.put("success", false);
		...
		// 创建图片记录信息
		TbAttachmentJsz tbAttachmentJsz = new TbAttachmentJsz();
		tbAttachmentJsz.setClId(vehicleInfoId);
		tbAttachmentJsz.setAttaName(holder.getImageName());
		tbAttachmentJsz.setUlUserId(userInfo.getId());
		tbAttachmentJsz.setUlUserName(userInfo.getRealName());
		tbAttachmentJsz.setUlTime(new Date());
		
		Execution<TbAttachmentJsz> execution = tbAttachmentJszService.uploadSingle(tbAttachmentJsz,oldImgId,holder);
