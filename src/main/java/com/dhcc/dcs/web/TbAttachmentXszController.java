 ...
 */
@Controller
@RequestMapping("/tbAttachmentXsz")
public class TbAttachmentXszController{

	 private final Logger logger = LoggerFactory.getLogger(this.getClass());
	// 支持上传证件详情图的最大数量
	private static final int IMAGEMAXCOUNT = 10;
	@Resource
    private TbAttachmentXszService tbAttachmentXszService;
	@Resource
    ...
    public JSONObject doAddOld(HttpServletRequest request,String cId,String imgFlag) {
    	...
    	// 3.添加附件信息
    	if (imgList.size() > 0) {
    		TbAttachmentXsz tbAttachmentXsz = new TbAttachmentXsz();
    		tbAttachmentXsz.setClId(cId);
    		tbAttachmentXsz.setUlUserId(user.getId());
    		tbAttachmentXsz.setUlUserName(user.getRealName());
    		try {
    			// 执行添加操作
    			Execution<TbAttachmentXsz> execution = tbAttachmentXszService
    					.add(tbAttachmentXsz, imgList);
    			...
    			json.put("success", false);
    			json.put("errMsg", e.toString());
    			logger.error("添加行驶证上传记录时出现异常！error message:"+e.getMessage(),e);
    			return json;
    		}
    	}
    	else {
	...
	public JSONObject uploadSingle(HttpServletRequest request) {
		...
		try {
			...
			logger.error("保存行驶证信息失败！error massage："+e.getMessage(),e);
			json.put("success", false);
		...
		// 创建图片记录信息
		TbAttachmentXsz tbAttachmentXsz = new TbAttachmentXsz();
		tbAttachmentXsz.setClId(vehicleInfoId);
		tbAttachmentXsz.setAttaName(holder.getImageName());
		tbAttachmentXsz.setUlUserId(userInfo.getId());
		tbAttachmentXsz.setUlUserName(userInfo.getRealName());
		tbAttachmentXsz.setUlTime(new Date());
		
		Execution<TbAttachmentXsz> execution = tbAttachmentXszService.uploadSingle(tbAttachmentXsz,oldImgId,holder);
