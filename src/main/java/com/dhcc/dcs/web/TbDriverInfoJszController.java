...
@Controller
@RequestMapping("/tbDriverInfoJsz")
public class TbDriverInfoJszController {
	...
	public JSONObject upload(HttpServletRequest request) {
		...
		List<ImageHolder> contractImgList = new ArrayList<ImageHolder>();
		CommonsMultipartResolver commonsMultipartResolver = new CommonsMultipartResolver(
				request.getSession().getServletContext());
		try {
			if (commonsMultipartResolver.isMultipart(request)) {
				handleImage(request, contractImgList);
			} else {
				// 如果不是编辑模式，那么要求必须上传图片
				if(isEdit) {
			...
			logger.error("保存司机驾驶证信息失败！error massage："+e.getMessage(),e);
			json.put("success", false);
		...
		// 如果图片文件列表不为空
		if(contractImgList.size()>0) {
			// 上传图片到服务器，并将图片信息保存至购销合同附件表中
			String driverInfoId = HttpServletRequestUtil.getString(request, "driverInfoId");
			Execution<TbDriverJsz> execution = tbDriverJszService.upload(
					driverInfoId,userInfo.getId(),userInfo.getRealName(),contractImgList);
	...
	public JSONObject uploadSingle(HttpServletRequest request) {
		...
		tbDriverJsz.setAttaName(holder.getImageName());
		tbDriverJsz.setUlUserId(userInfo.getId());
		tbDriverJsz.setUlUserName(userInfo.getUserName());
		tbDriverJsz.setUlTime(new Date());
		
		Execution<TbDriverJsz> execution = tbDriverJszService.uploadSingle(tbDriverJsz,oldImgId,holder);
